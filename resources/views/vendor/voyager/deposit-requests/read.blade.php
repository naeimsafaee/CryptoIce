...
@section('page_header')
    <div class="action">
        ...
        {{--@can('delete', $dataTypeContent)
            ...
            @else
                <a href="javascript:;" title="{{ __('voyager::generic.delete') }}" class="btn btn-red btn-danger delete"
        ...
        <a class="btn btn-red btn-danger delete" href="{{ route('reject_deposit_request' , $dataTypeContent->id) }}">
            <span><i class="voyager-exclamation"></i></span>&nbsp;
            Reject
        </a>

        <a class="btn btn-warning" href="{{ route('accept_deposit_request' , $dataTypeContent->id) }}">
            <span><i class="voyager-thumbs-up"></i></span>&nbsp;
            Accept
        </a>

        @php
            if($dataTypeContent->network_id === 1){
        ...
        <a class="btn btn-warning" href="{{ $url . $dataTypeContent->tx_id }}">
            <span><i class="voyager-thumbs-up"></i></span>&nbsp;
            Explorer
        </a>
            ...
            <div class="box">
                <div class="panel-body" style="padding-top:0;">
                    ...
                    @elseif($row->type == 'relationship')
                        @if($row->field === 'deposit_request_belongsto_coin_price_relationship')
                            @php
                                $coin_name = \App\Models\CoinPrice::query()->findOrFail($dataTypeContent->coin_price_id)->coin->coin_name;
                            @endphp
                            {{ $coin_name }}
                        @else
