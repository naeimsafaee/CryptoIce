...
<tr>
    <td class="chid1" @if($coin->coin_price->is_favorite) onclick="remove_from_favorite({{ $coin->coin_price->id }} , this)" @else onclick="add_favorite({{ $coin->coin_price->id }})" @endif>

        @if($coin->coin_price->is_favorite)
    ...
    <td data-label="Market">
        <div class="flex-box market-item">
            <div class="flex-box icon-box">
                <img style="margin:8px" src="{{ asset("assets/icon/crypto/" . $coin->coin_price->coin->symbol . '.svg') }}"
                     alt="alt text"/>
            </div>
            <div class="content">
                {{ strtoupper($coin->coin_price->coin->symbol) }} <span>({{ ucfirst($coin->coin_price->coin->coin_name) }})</span>
            </div>
        </div>
    </td>
    <td class="sell-buy " data-label="Price">
        <div class="flex-box">
            <div>
                <span>
                    ${{ sprintf( "%.5f", $coin->coin_price->price * $coin->amount) }}
                </span>
    ...
    <td class="sell-buy " data-label="Amount">
        <div class="flex-box">
            <div>
                <span>
                     {{ sprintf("%.5f", $coin->amount) }}
                </span>
    ...
    <td>
        <div class="flex-box sell-by-buttons">
            @if($coin->coin_price->coin->symbol !== "usdt")
                <a href="{{ route('trade' , ['trade' => strtoupper($coin->coin_price->coin->symbol ). "_USDT"]) }}" class="button">
                    {{ text('Trade') }}
                </a>
            @endif
        </div>
