                    ...
                    <div class="padding-items col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="border-item">
                            <div class="row margin profile-row">
                                <div class="margin-bottom padding-items col-lg-6 col-md-12 col-sm-12 col-12"
                                     style="flex: 2">
                                ...
                                <div class="margin-bottom padding-items col-lg-3 col-md-12 col-sm-12 col-12"
                                     style="flex: 1">
                                    <a @if(!$is_copied) data-toggle="modal" data-target="#follow-pay-popup" @endif
                                       class="@if($is_copied) Copy-white-button button @else dark-button @endif flex-box" onclick="handleGetCopyId(event)">
                                        @if($is_copied)
                                            Copied
                                        @else
                                            Copy
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="margin col-lg-12 col-md-12 col-sm-12 col-12">
                                            <div class=" result flex-box justify-content-start">
                                                <p class="red">
                                                    {{ $last_7_days_copiers_count - $last_last_7_days_copiers_count }}
                                                    ({{ ($last_7_days_copiers_count - $last_last_7_days_copiers_count) / ($last_last_7_days_copiers_count !== 0 ? $last_last_7_days_copiers_count : 1) * 100 }}
                                                    %)
                                                </p>
                                                <p class="grey">
                                                    Copiers last 7d
...
@section('javascript')
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [
                        {{ $last_last_7_days_copiers_count }},
    ...
    <script>
        let TraderId = {{ $client->trader->id }};

        function handleGetCopyId(event) {

            $("#avatar_modal").attr('src', "{{ $client->avatar ? Voyager::image($client->avatar) : asset('assets/icon/user.svg') }}");
            $("#fullname_modal").text('{{ $client->full_name }}');
            $("#username_modal").text('{{ $client->username }}');
            $("#percent_modal").text("{{ $client->trader->percent }}%");
            $(".price_modal").text($(event.target).data('price') + "$");
            $("#description").text('{{ $client->trader->description }}');
